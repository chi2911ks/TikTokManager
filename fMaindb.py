...
class TiktokManager(Ui_MainWindow):
    ...
    def addGroup(self):
        name, chk = QtWidgets.QInputDialog().getText(None, "Tạo thư mục", "Tên thư mục:")
        if chk:
    ...
    def editGroup(self):
        ...
        if index != 0:
            name, chk = QtWidgets.QInputDialog().getText(None, "Sửa tên thư mục", "Tên thư mục mới:", text=old)
            if chk:
    ...
    def deleteGroup(self):
        index = self.cbGroup.currentIndex()
        if index != 0:
            button = self.MsgBox("Bạn cho chắc chắn muốn xoá ?", QtWidgets.QMessageBox.Question)
    ...
    def searchItem(self):
        index = {"username": 1, "password": 2, "mail": 3, "passmail": 4, "cookie": 5, "note": self.col_note, "description": self.col_description}
    ...
    def refreshTable(self):
        ...
        if typeText == '[Tất cả thư mục]': typeText=None
    ...
    def MsgBox(self, text="", icon=QtWidgets.QMessageBox.Information, standar=QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No):
    ...
    def OpenFolderAvatar(self):
        folder = QtWidgets.QFileDialog.getExistingDirectory(None, "Mở folder chứa ảnh", "./")
        if folder != "": self.fUpdateInfo.locationAvatar.setText(folder)
    def setWidthColumnTableWidget(self):
        ...
        self.tableWidget.horizontalHeader().setSectionResizeMode(self.col_status, QtWidgets.QHeaderView.Stretch)
        self.tableWidget.horizontalHeader().setSectionResizeMode(self.col_btn, QtWidgets.QHeaderView.Fixed)
    ...
    def addAccounts(self):
        ...
        if data != 0:
            ...
            for data in data:
                ...
                if len(dt) == 5: cookie = dt[-1]
                value = dt[0], dt[1], dt[2], dt[3], cookie, "", "", self.fAddAccounts.cbGroup.currentText()
                username, password, mail, passmail, cookie, note, description, nameType = value
                sql_add_query = 'INSERT INTO accounts (username, password, mail, passmail, cookie, note, description, type) VALUES ("%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s")'%(username, password, mail, passmail, cookie, note, description, nameType)
                try:
    ...
    def addItemAddAccounts(self, value, add=False):
        ...
        username, password, mail, passmail, cookie, note, description, nameType = value
        if nameType != self.cbGroup.currentText() and self.cbGroup.currentText() != "[Tất cả thư mục]" and add: self.tableWidget.hideRow(row)
    ...
    def createButton(self, row: int):
        ...
        button.setStyleSheet(StyleSheet.QPushButton%("#2E8B57", "#2E8B57", "#90EE90", "#90EE90", "#00FA9A", "#00FA9A"))
    ...
    def buttonClicked(self, row):
        ...
        if button.text() == "Bắt đầu":
            ...
            button.setStyleSheet(StyleSheet.QPushButton%("#FF6347", "#FF6347", "#FF0000", "#FF0000", "#FF0000", "#FF0000"))
    ...
    def contextMenuEvent(self, event):
        ...
        for value in vars().values():
            if isinstance(value, QtWidgets.QMenu): value.setStyleSheet(StyleSheet.QMenu)
        ...
        exports_account_action = QtWidgets.QAction("Xuất file username|password|mail|passmail|cookie")
        ...
        for col in range(1, 10):
            if col == 7 or col == 6: continue
            col_name = self.tableWidget.horizontalHeaderItem(col).text()
            action = QtWidgets.QAction(col_name)
            action.triggered.connect(lambda _, c=col, c_n=col_name: self.editItemAction(c, c_n))
            items.append(action)
    ...
    def editItemAction(self, col, col_name):
        ...
        valueNew, chk = QtWidgets.QInputDialog().getText(None, "Cập nhật thông tin "+col_name, col_name+" mới: ", text=self.tableWidget.item(row, col).text())
        if chk:
    ...
    def getRowSelected(self):
        ...
        for row in range(self.tableWidget.rowCount()):
            ...
            if item.checkState() == Qt.Checked: rows.append(row)
        return rows
    ...
    def DeleteProfile(self):
        ...
        for row in self.getRowSelected():
            ...
            except: self.show(row, self.col_status, "Xoá profile thất bại!")
    def DeleteAcc(self):
        usernames = []
        for row in sorted(self.getRowSelected(), reverse=True):
            username = self.tableWidget.item(row, 1).text()
        ...
        self.labelCountSelect.setText(self.stringCountSelect%self.tableWidget.rowCount())
...
class RunTiktok:
        ...
        # if self.login.checkLogin(): 
            if self._this.fUpload.rbOnlyVideo.isChecked():
                ...
                if location != "" or os.path.exists(location):
                    ...
                    self.show.emit(self._row, self._this.col_status, "Không có đường dẫn này!")
            else:
                ...
                while index < self._this.fSetting.countVideo.value():
                    if not self._this.dirsVideo: 
                        self.show.emit(self._row, self._this.col_status, "Hết video trong folder đó!")
                        break
                    ...
                    if path.endswith(".mp4") or path.endswith(".webm"):
                        ...
                        if self._this.fSetting.cbDeleteVideo.isChecked() and chkUp:
                            self.show.emit(self._row, self._this.col_status, "Đang xoá video...")
                            while True:
                ...
                self.show.emit(self._row, self._this.col_status, "Upload thành công %s video!"%success)
    ...
    def _follow(self):
        for link in self._this.fSeeding.textLinkUser.toPlainText().splitlines():
            ...
            self.show.emit(self._row, self._this.col_status, "Theo dõi thành công %s !"%link)
    ...
    def run(self):
        ...
        try:
            ...
            if rbLogin():
                ...
                else: self.show.emit(self._row, self._this.col_status, "Đã đăng nhập!")
            else:
                chkContinue = True
                if not self.login.checkLogin():
                    if cookie != "": 
                ...
                if chkContinue:
                    if rbUpload(): self._uploadVideo()
                    elif rbUpdateInfo():
                        
                        # if self.login.checkLogin(): 
                            self.show.emit(self._row, self._this.col_status, "Đang cập nhật thông tin nick...")
                            ...
                            if self._this.fUpdateInfo.gbAvatar.isChecked():
                                self.show.emit(self._row, self._this.col_status, "Đang cập nhật ảnh đại diện...")
                            ...
                            if self._this.fUpdateInfo.gbBio.isChecked():
                                self.show.emit(self._row, self._this.col_status, "Đang cập nhật tiểu sử...")
                                ...
                                if os.path.exists(pathBio): 
                                    with open(pathBio, encoding="utf-8") as file:
                                        ...
                                        for i in range(5):
                                            ...
                                            if len(bio) > 80: continue
                                            else: 
                                                self.update.updateBio(bio)
                                                break
                                        
                            if self._this.fUpdateInfo.gbName.isChecked():
                                self.show.emit(self._row, self._this.col_status, "Đang cập nhật tên và tiktokID...")
                                ...
                                if os.path.exists(pathName): 
                                    ...
                                    if self._this.fUpdateInfo.cbTiktokID.isChecked():
                                        ...
                                        self.usernameNew = self.update.updateTikTokID(self.usernameNew)

                            sleep(5)
                            self.login.ClickJsWebElement('xpath', '//*[text()="Lưu"]')
                            sleep(1)
                            try: self.login.ClickJsWebElement('xpath', '//button[@data-e2e="set-username-popup-confirm"]', timeout=5)
                            except: pass
                            sleep(10)
                            ...
                            self.show.emit(self._row, self._this.col_status, "Cập nhật thông tin nick thành công!")
                    elif rbSeeding():
                    # if self.login.checkLogin(): 
                        ...
                        if fSeeding.groupBoxLinkVideo.isChecked():
                            for link in self._this.fSeeding.textLinkVideo.toPlainText().splitlines():
                                ...
                                for i in range(random.randint(self._this.fSeeding.spinBoxTimeView.value(), self._this.fSeeding.spinBoxTimeView_2.value()), -1, -1):
                                    self.show.emit(self._row, self._this.col_status, "Xem video trong %s giây thì kết thúc..."%i)
                                    sleep(1)
                                if fSeeding.cbView.isChecked(): self.show.emit(self._row, self._this.col_status, "Xem xong video!")
                                ...
                                for i in range(random.randint(self._this.fSeeding.spinBoxDelay.value(), self._this.fSeeding.spinBoxDelay_2.value()), -1, -1):
                                    self.show.emit(self._row, self._this.col_status, "Vui lòng đợi %s giây nữa..."%i)
                                    sleep(1)
        ...
        if self._this.fUpdateInfo.cbTiktokID.isChecked() and rbUpdateInfo() and chkUpd:
            ...
            while True:
                try: 
                    os.rename(self.login.profile, os.path.abspath(os.path.join("profile", self.usernameNew)))
                    break
                except: pass
