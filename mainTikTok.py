...
class TikTok(CapchaTiktok):
    ...
    def __init__(self, this, row, username, password, cookie, api_key, x, y, proxy) -> None:
        ...
        self.getDriver(profile=self.profile, x=self.x, y=self.y, width=500, height=700, version=self.this.version_main, proxy=proxy, headless=self.this._this.fSetting.rbHidenChrome.isChecked(), chromedriver=os.path.abspath("ChromeDrivers\\103.exe"), binary_location=os.path.abspath("Data\\orbita-browser\\chrome.exe"))
