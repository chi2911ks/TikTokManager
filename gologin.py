...
class Gologin:
    ...
    def _createFolder(self, name):
        if not os.path.exists(self.pathProfile):
            os.mkdir(self.pathProfile)
    ...
    def changeProxy(self, profile, proxy):
        try:
            if proxy:
                ip, port, username, passwword = proxy.split(":")
            else:
                ip = port = username = passwword = ""
            data = json.loads(open(profile + "\Default\Preferences").read())

            proxy = data["gologin"]["proxy"]
            proxy["host"] = ip
            proxy["password"] = passwword
            proxy["port"] = port
            proxy["username"] = username
            print(proxy)
            data["gologin"]["proxy"] = proxy
            json_data = json.dumps(data)
            with open(profile + "\Default\Preferences", "w") as json_file:
    ...
    def createProfile(self, nameProfile):
        ...
        self.preferences = self.change(
            self.preferences, "tb_name_profile_SSL", nameProfile
        )
        self.preferences = self.change(
            self.preferences,
            "tb_user_agen",
            self._random_infotxt(os.path.abspath("Data\\UA\\Chrome.txt")),
        )
        ...
        dpdm_values = ["500", "700"]
        self.preferences = self.change(
            self.preferences, "tb_do_phan_dai_man_hinhx", dpdm_values[0]
        )
        self.preferences = self.preferences.replace(
            "tb_do_phan_dai_man_hinhy", dpdm_values[1]
        )

        self.preferences = self.change(
            self.preferences,
            "hardwareChanger",
            random.choice("2|4|8|16|32".split("|")).strip(),
        )
        ...
        self.preferences = self.change(
            self.preferences, "tb_video_inputs", str(random.randint(1, 2))
        )
        self.preferences = self.change(
            self.preferences, "tb_audio_inputs", str(random.randint(0, 2))
        )
        self.preferences = self.change(
            self.preferences, "tb_audio_outputs", str(random.randint(1, 2))
        )
        self.preferences = self.change(
            self.preferences, "tb_webGL_vendor", "Google Inc. (Intel)"
        )
        self.preferences = self.change(self.preferences, "tb_web_Renderer", GPU())

        self.preferences = self.change(
            self.preferences, "tb_web_mo_cung_trinh_duyet", ""
        )
        ...
        self.preferences = self.change(
            self.preferences,
            "sqzdbzkff4zv8ev9mgm1a6m8xw8xucyeyqjo1bibdjph9cechdqkhocecu",
            num4,
        )
        ...
        self.preferences = self.change(
            self.preferences, "PrevNavigationTimeChange", num9
        )
